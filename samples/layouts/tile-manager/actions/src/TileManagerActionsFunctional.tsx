...
// Icon registration is a one-time module-level side effect (no state needed)
...
export default function Actions() {
    // useCallback ensures stable handler references across renders
    const onCustomOneClick = useCallback((event: React.MouseEvent) => {
        const tile = (event.currentTarget as HTMLElement).closest('igc-tile') as any;
        if (!tile) return;
        ...
        if (tile.maximized) {
            ...
            tile.querySelectorAll('.additional-action').forEach((btn: Element) => btn.remove());
        }
    }, []);

    const onCustomTwoClick = useCallback((event: React.MouseEvent) => {
        const tile = (event.currentTarget as HTMLElement).closest('igc-tile') as any;
        if (!tile) return;
